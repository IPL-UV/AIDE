...
class ClassificationVisualizer3D(VisualizerAbstract):
    """
   Visualization for 2D outputs
    """

    def __init__(self, config, model, dataloader):
        """Initialization of the ClassificationVisualizer3D's parameters
        ...
        self.data_classes = [i for i in range(1, config['data']['num_classes'])]
        ...
        self.test_outputs_mean_when_event = dict.fromkeys(self.data_classes, [])
        self.test_outputs_mean_without_event = dict.fromkeys(self.data_classes, [])
    ...
    def global_operations(self):
        ...
        """
        misc.plot_confusion_matrix(self.cm, torch.Tensor(self.test_outputs), torch.Tensor(self.test_labels).int(), self.save_path)
        if self.config['evaluation']['visualization']['params']['time_aggregation']:
            for num_class in self.data_classes:
